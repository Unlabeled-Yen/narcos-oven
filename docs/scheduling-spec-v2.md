...
- **爆量時**：週二可挪部分時間加工製作（Yen 2026-07-03 confirm、彈性上限 +8 hr）
- **出貨日 (batchDate) = 週二** 或**週三**（客人視角）
- **實際製作日 = 出貨日前 3-4 天的週六/日/一**
- **週四之前訂購**才能進**下週批**（前置期 ≥ 5 天、待 Yen final confirm 天數）
...
#### 巴斯克類（7 個 atom、時間統一）✅ Yen confirm (2026-07-03)

**Yen 明確答**：7 種口味時間全相同（選項 A）。

| atom | 1 爐 | 2 爐 | 3 爐 | 每爐容量 |
|---|---|---|---|---|
| 原味巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 白玉原味巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 芝麻巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 白玉芝麻巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 焙茶巴斯克（純焙茶） | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 焙茶栗子巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |
| 白玉烏龍茶巴斯克 | **2.0 h** | **3.5 h** | **5.0 h** | 4 顆 |

#### 香料堅果醬（3 個 atom、時間依總量、非容量）✅ Yen confirm (2026-07-03)

**Yen 明確答**：時間跟包裝容量無關、只跟**總醬量**有關（**選項：時間相同、只是分裝不同**）。

**計算規則**：每 1000ml 醬料需要 2 hr。90ml 和 240ml 只是包裝規格不同、製作時間看**總 ml 數**：
- 需要 20 罐 90ml = 1800 ml → 2 × 1800/1000 = 3.6 hr
- 需要 5 罐 240ml = 1200 ml → 2 × 1200/1000 = 2.4 hr
- 混合：10 罐 90ml + 3 罐 240ml = 900+720 = 1620 ml → 3.24 hr

| atom | 說明 |
|---|---|
| 香料堅果醬 40ml | 目前菜單無販售、若有訂單依總 ml 計算 |
| 香料堅果醬 90ml | 依 batch 總 ml 數計、非罐數 |
| 香料堅果醬 240ml | 依 batch 總 ml 數計、非罐數 |

#### 瑕疵小脆捲（1 個 atom）✅ Yen confirm (2026-07-03)

**Yen 明確答**：對、是肉桂捲副產品、不佔製作時間。

| atom | 時間 | 說明 |
|---|---|---|
| 瑕疵小脆捲 10 顆 | **0 h** | 肉桂捲製作副產品、不佔額外製作時間 |

**Pipeline 影響**：`estimateProductionHours()` 遇到瑕疵小脆捲 SKU 直接 skip、時間貢獻 = 0。
...
## 3. batchDate 語意重新定義 ✅ Yen confirm (2026-07-03)

**Yen 明確答**：
- batchDate 對 = 週二為主（週三 fallback）
- 賣貨便「⚠️必填！指定出貨日 7/07（二）」的 7/07 = **寄出日**（不是客人收到）

**確認結論**：
- v2 batchDate = **週二寄出日**（客人視角 = 「這天我的貨會出」）
- 客人約週三或週四會收到（配送 1-2 天）
- 現有系統 batchDate = 週二 = 完全相容、v2 沿用
...
規則（Yen 補充 2026-07-03）：
1. 客人 wish_date 存在 → 系統**先試該日**
2. 該日時間預算足夠 → 排入、`assignment_source = customer_wish_kept`
3. 該日產能不足 → **順延到最遠可行日**（不進 unscheduled、除非 10 週全滿）
4. Yen 在 UI 看到「客人希望 X、系統排 Y、差 N 週」可介入改回、或通知客人
...
## 5. RC-2：最低前置期規則 ✅ Yen confirm (2026-07-03)
...
## 7. RC-6：超載處理（純 FIFO、無特殊訂單保住）

Yen 明確（2026-07-03）：**沒有特殊訂單保住規則**（例如金額大的保住？→ 沒這規則）。純 FIFO。
...
### 7.2 唯一例外：Pre-booked 巴斯克 strict 不能被 push

`assignment_source=customer_wish_kept` 且 `wish_priority=strict`（= 巴斯克）的訂單、**受憲章 #13 保護**、絕對不被 push。**其他所有訂單一律 FIFO**（含金額大的、含 KOL、含 VIP 等——都無例外）。
...
- §2.1 巴斯克 7 口味時間統一 (2/3.5/5 hr、4 顆容量)
- §2.1 香料堅果醬時間依 batch 總 ml 計、非容量（1000ml=2hr）
- §2.1 瑕疵小脆捲 = 肉桂捲副產品、時間 0h

### ✅ 已 confirm (2026-07-03) 追加
- §2.1 蘋果/焦糖/磅蛋糕 3 爐 + 4 爐+ 斜率：**暫用我推導的數字**（實際跑起來後 Yen 可微調）
- §3 batchDate = 週二為主、賣貨便「指定出貨日」= 寄出日
- §4.1 巴斯克 strict 找不到位子 → **排最遠可行日**（不 unscheduled）
- §7 超載處理：純 FIFO、**無特殊訂單保住**（金額大不例外）
- §1 週二可加工彈性：**爆量時允許**（+8 hr 上限）

### ✅ 已 confirm (2026-07-03) 最後一項
- §5 前置期 = **5 天**

### 🎉 Spec 全數 confirm、可動 M6.5 實作

**下一步**：進入 §11 M6.5 實作 milestone (7 sub-tasks、估 7 hr)。
